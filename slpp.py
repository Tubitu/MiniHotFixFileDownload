...
# 用于解析lua table数据
# https://github.dev/IlyaSkriblovsky/slpp-23
...
class SLPP(object):
        ...
        if self.ch and self.ch == '}':
            ...
            while self.ch:
                ...
                if self.ch == '{':
                    ...
                    if k is not None:
                        o[idx] = k
                    if not numeric_keys and len([key for key in o if isinstance(
                            key, six.string_types + (float, bool, tuple))]) == 0:
                        ar = []
                        for key in o:
                            ar.insert(key, o[key])
                        o = ar
    ...
    def word(self):
        ...
        if self.ch != '\n':
            s = self.ch
        self.next_chr()
        while self.ch is not None and self.alnum.match(
                self.ch) and s not in self.words:
            s += self.ch
            self.next_chr()
